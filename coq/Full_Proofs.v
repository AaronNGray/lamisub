...
Require Import Full_ott Full_Infra.
...
Proof.
  ...
  asserts* W1: (wf (G & y ~ bind_sub e0 A0)).
  destruct (wf_to_wt W1) as [? ?].
  apply (@sub_prod (L \u L0)). auto. auto. auto. intros.
  apply* H1. intros. apply* H1. auto. auto.
  pick_fresh y.
  lets: H5 y __. auto. asserts* W: (wf (G & y ~ bind_sub e' A')).
  destruct (wf_to_wt W) as [? ?].
  apply (@sub_prod (L \u L0 \u L1)). apply* IHW2. apply* IHW1.
  auto.
  ...
  apply_empty~ (@sub_narrowing_aux e).
  unfold transitivity_on. intros. apply* IHW1.
  apply* H5. auto. auto.
  clear IHS2_2.
  lets: IHS2_1 S1_4 H1 S1_3 H0 IHW2.
  apply* H.
  (* case: castup *)
...
Proof.
  ...
  (* case: prod *)
  left. apply value_prod. auto. auto. apply_fresh lc_trm_prod as y.
  auto. auto.
